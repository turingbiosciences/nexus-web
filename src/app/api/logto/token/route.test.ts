...
describe('Token API Route Security Test', () => {
  ...
  it('should not leak sensitive upstream error details in Error message', async () => {
    ...
    // If the error message contains the sensitive info, verify it's NOT leaked
    if (loggedError.message.includes(sensitiveError)) {
      // console.log('VULNERABILITY CONFIRMED: Error message contains sensitive info:', loggedError.message);
    }
