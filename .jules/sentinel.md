...
## 2026-03-17 - Rate Limiting OAuth Callbacks\n**Vulnerability:** Missing rate limiting on the `sign-in-callback` OAuth endpoint exposed the application to DoS attacks via excessive identity provider redirects.\n**Learning:** When implementing rate limiting on browser-facing OAuth callbacks, returning JSON breaks the user experience, while using a 302 redirect for rate limits is an anti-pattern that can worsen client spamming.\n**Prevention:** Return a user-friendly HTML response with a 429 Too Many Requests status code for browser-based endpoints that hit rate limits.
