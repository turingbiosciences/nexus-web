...
**Learning:** Next.js `NextRequest.ip` is the most reliable way to get client IP in Vercel/Edge environments. If falling back to headers, always take the *first* IP from `x-forwarded-for` (client IP) rather than the whole string, as proxies append to it.
